...
use binance::futures::rest_model::ContractType;
use binance::rest_model::SymbolStatus;
...
async fn main() {
    ...
    // private api
    // user_stream().await;
    // user_stream_websocket(keep_running.clone()).await;
    tokio::spawn(Box::pin(user_stream_websocket(keep_running.clone())));
    // public api
    let mut streams: Vec<BoxFuture<'static, ()>> = Vec::new();

    let exchange_info = general.exchange_info().await.unwrap();
    for symbol in exchange_info.symbols {
        if symbol.status == SymbolStatus::Trading && symbol.contract_type == ContractType::Perpetual {
            // streams.push(Box::pin(market_websocket(
            //     symbol.symbol.clone(),
            //     logger_tx.clone(),
            //     keep_running.clone(),
            // )));
            // streams.push(Box::pin(depth_websocket(
            //     symbol.symbol,
            //     logger_tx.clone(),
            //     keep_running.clone(),
            // )));
    ...
    for stream in streams {
        handles.push(tokio::spawn(stream));
    }
...
#[allow(dead_code)]
async fn user_stream_websocket(keep_running: Arc<AtomicBool>) {
    let user_stream: UserStream = Binance::new_with_env(Config::default());
...
#[allow(dead_code)]
async fn depth_websocket(asset: String, logger_tx: UnboundedSender<WebsocketEvent>, keep_running: Arc<AtomicBool>) {
    let stream: String = depth_stream(&asset.to_lowercase(), 10);
    let mut web_socket: WebSockets<'_, WebsocketEvent> = WebSockets::new(|event: WebsocketEvent| {
        if let Err(_) = logger_tx.send(event) {}
        Ok(())
    });

    web_socket.connect(&stream).await.unwrap(); // check error
    if let Err(e) = web_socket.event_loop(&keep_running).await {
        println!("depth_websocket Asset: {asset}, Error: {e}");
    }
    web_socket.disconnect().await.unwrap();
    println!("depth_websocket disconnected {asset}");
}

#[allow(dead_code)]
async fn market_websocket(asset: String, logger_tx: UnboundedSender<WebsocketEvent>, keep_running: Arc<AtomicBool>) {
    let agg_trade: String = agg_trade_stream(&asset.to_lowercase());
    let mut web_socket: WebSockets<'_, WebsocketEvent> = WebSockets::new(|event: WebsocketEvent| {
        if let Err(_) = logger_tx.send(event) {}
        Ok(())
    });
    ...
    if let Err(e) = web_socket.event_loop(&keep_running).await {
        println!("market_websocket Asset: {asset}, Error: {e}");
    }
    web_socket.disconnect().await.unwrap();
    println!("market_websocket disconnected {asset}");
}
